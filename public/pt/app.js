...
cameraBtn.addEventListener('click', async () => {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        alert('Seu navegador não permite acesso à câmera. Use HTTPS ou localhost.');
    ...
    try {
        ...
        if (err.name === 'NotFoundError' || err.message.includes('not found')) {
            msg = 'Nenhuma câmera conectada foi detectada. Se estiver no PC, conecte uma webcam.';
        } else if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError') {
            msg = 'Permissão negada. Você deve permitir o acesso à câmera na barra de endereço.';
        } else if (err.name === 'NotReadableError') {
            msg = 'A câmera está sendo usada por outro aplicativo (Zoom, Meet, etc).';
        }

        alert(`${msg}\n\nDetalhe técnico: ${err.message || err.name}`);
        cameraModal.style.display = 'none';
    }
});
...
async function generateImage() {
    ...
    const nombre = window.nombreUsuario || 'Usuário';
...
INTEGRAÇÃO (muito importante):
- Não cole a foto original como um recorte plano sobre o fundo.
- Renderize novamente o corpo, pose e iluminação do jogador para que a pessoa faça parte natural da cena do estádio: mesma direção de luz, sombras, tom de cor e nível de detalhe que o resto da carta.
- O resultado deve parecer uma única fotografia coesa do jogador, não uma montagem de um rosto recortado sobre uma ilustração.
...
QUALIDADE: alta resolução, ultra detalhado, fotorrealista. Não caricatura, não anime, não desenho.`;
    ...
    try {
        ...
        showToast(error.message || 'Erro ao gerar a imagem', 'error');
    } finally {
...
async function openCamera() {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        alert('Seu navegador não permite acesso à câmera. Use HTTPS.');
        cameraModal.style.display = 'none';
        return;
    }
    ...
    try {
        ...
        console.error('Error de cámara:', err);
        alert('Erro ao abrir a câmera: ' + (err.message || err.name));
        cameraModal.style.display = 'none';
    }
}
...
async function checkApiHealth() {
    try {
        ...
        if (!data.hasApiKey) {
            showToast('⚠️ Configure sua GOOGLE_API_KEY no arquivo .env', 'error');
        }
    } catch (error) {
